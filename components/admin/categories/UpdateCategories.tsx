...
const UpdateCategory = ({ setCategories }: any) => {
    ...
    const submitHandler = async () => {
        try {
            const { data } = await axios.post("/api/admin/category/create",{ name });
            console.log(data)
            ...
            >
                {(form) => (
                    <Form className="flex-1 flex-col">
                        ...
                        />
                        <button className="mt-4 bg-green-500 py-2 px-4 rounded text-white">
                            Update category
                        </button>
                    </Form>
                )}
